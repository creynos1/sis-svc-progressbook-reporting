...
{
    public class SpecialServicesReportViewInfoRepository : AdoRepository<ReportViewInfo>
    {
        public SpecialServicesReportViewInfoRepository()
            :base(new DbCommandExecutor(ConfigurationManager.ConnectionStrings["SpecialServices"].ConnectionString))
        {
        }

        public List<ReportViewInfo> GetSpsEntities()
        {
            var query = @"
                SELECT
	                ViewName,
